...
func NewDoctorManager(baseDir string) *DoctorManager {
	return &DoctorManager{baseDir: baseDir}
}
...
func (dm *DoctorManager) RunFixes() FixResult {
	...
	// Fix: seed ~/.orbital.env if missing.
	...
	if err == nil {
		...
		if _, err := os.Stat(envFile); os.IsNotExist(err) {
			...
			if androidHome != "" {
				content := fmt.Sprintf("# Orbital configuration (auto-generated by orbital doctor --fix)\nANDROID_HOME=%s\n", androidHome)
				if err := os.WriteFile(envFile, []byte(content), 0o644); err != nil {
					fixes = append(fixes, Check{
						Name:    "create ~/.orbital.env",
						Status:  StatusFail,
						Message: fmt.Sprintf("failed to write %s: %v", envFile, err),
					})
				} else {
					fixes = append(fixes, Check{
						Name:    "create ~/.orbital.env",
						Status:  StatusOK,
						Message: fmt.Sprintf("created %s with ANDROID_HOME=%s", envFile, androidHome),
					})
				}
			}
		}
...
func (dm *DoctorManager) checkAndroidHome() []Check {
	...
	if androidHome == "" {
		checks = append(checks, Check{
			Name:    "ANDROID_HOME",
			Status:  StatusFail,
			Message: "ANDROID_HOME is not set (set it in ~/.orbital.env)",
			Fixable: true,
		})
		return checks
	}
...
func (dm *DoctorManager) checkOrbitalEnv() Check {
	home, err := os.UserHomeDir()
	if err != nil {
		return Check{Name: "~/.orbital.env", Status: StatusWarn, Message: "cannot determine home directory"}
	}
	...
	if _, err := os.Stat(envFile); os.IsNotExist(err) {
		return Check{
			Name:    "~/.orbital.env",
			Status:  StatusWarn,
			Message: "config file not found — using environment defaults",
			Fixable: true,
		}
	}
